...
processing ${layer}[${layerNs}] @ ${this.baseDir}
    ...
    public void generate(){
        println("${this.class.name} starting ...")
        if( canGenerate() ) {
            preGenerate()
            generateSources()  // 生成源代码
            postGenerate()
        }
        println("${this.class.name} ok!")
    }

    // 判断是否可以生成
    boolean canGenerate(){
        def build_flag = options["build-${layer}"]
        println("check flag build-${layer} = $build_flag , valid value is 'force' | 'skip' ")

        if( ! targetDir.isDirectory() ) return true

        // 如果强制 build
        if( build_flag == "force" ) return true;

        // 如果强制跳过
        if( build_flag == "skip" ) return false;
    ...
    void link(String srcDir, String linkDir){
        ...
        if( System.getProperty("os.name").toLowerCase().indexOf("windows") >= 0 ) {
            shellscript(shell: "cmd.exe", arg: "/c call ", tmpsuffix: ".bat", dir: "${parentDir}", osfamily: "windows", failonerror: "false", """
                mklink /j ${dirname} "${srcDir.winpath()}"
                if not "%errorlevel%" == "0" (
				    echo "can not found mklink.exe, try xcopy "
                    rem xcopy /e /y /i /q "${srcDir.winpath()}" "${dirname}"
                )
            """)
        }else {
            shellscript(shell:"bash", dir: "${parentDir}", osfamily:"unix", failonerror:"false", """
...
class AnalysisGenerator extends Generator{
    ...
    public final static String PG_PATTERN = /\b(pg\-[\w\d\-]+)\s*:\s*([\w\-\d\s,]+\s*)/
...
""")
    ...
    def copyPomXml(String fromPom, String toPom, def nodes, def dependencies,  @ClosureParams(value=SimpleType.class, options="groovy.util.Node") Closure<Boolean> callback = null){
        ...
        if( dependencies != null ) dependencies.each{
            ...
            if( !(grp == null && art == null && ver == null) ){
                ...
                if( type != null ) dep.appendNode("type", type)
            }
        }
    ...
    def removePomNode(def pom, def nodes){
        if ( pom instanceof Collection ) pom = pom[0]
    ...
    def removeNodeChild(def parentNode, def callbackIf){
        assert !(parentNode instanceof Collection) || parentNode.size() == 1

        if(parentNode instanceof Collection && parentNode.size() == 1){
            parentNode = parentNode[0]
        }

        def nodes = []
        parentNode.children().each{
            if( callbackIf(it) == true ) nodes += it
        }

        nodes.each{
            removePomNode(parentNode, it)
        }
    }
...
// service * with serviceImpl
    ...
    void extractJavaFiles(entityName, entityOptions, relationNames, reverseRelationNames, absfiles){
        ...
        boolean isAggRoot = (entityOptions["pg-root"] ?: "") == "-"
        def classMaps = [
        ...
        // 如果是聚合根，则增加 Resource 处理
        if( isAggRoot ){
            // 移动 Resource 的命名空间
            classMaps += [ from:"${this.groupId}.web.rest.${entityName}Resource", to:"${this.groupId}.rest.${entityName}Resource"]
        }
        ...
        replaceClass("$tempSrcJavaDir", classMaps, files){
            ...
            // 去掉 pg-xxx 的所有内容，避免污染代码
            replaceText(PG_PATTERN, "", "gm", files)
        }
        ...
        if( isAggRoot ) assert "$tempSrcJavaDir/${groupId.packageToPath()}/rest/${entityName}Resource.java".isFile()
        ...
        if( isAggRoot ) incfiles += "${this.groupId}.rest.${entityName}Resource".packageToPath() + ".java"
        copyAndReplaceFiles(subdir1, [
                [dir: "$tempSrcJavaDir", includes: incfiles]
        ])

        assert "$subdir1/${groupId.packageToPath()}/dto/${entityName}DTO.java".isFile()
        if( isAggRoot ) assert "$subdir1/${groupId.packageToPath()}/rest/${entityName}Resource.java".isFile()
    ...
    void postProcess(){
        ...
        // 复制 web/rest 目录下的所有子目录
        ant.copy(todir: "${subProjectDir}/${artifactId.dashCase()}-web/src/main/java/${projectGroupId.packageToPath()}/web/rest/"){
            ant.fileset(dir:"$tempSrcJavaDir/${groupId.packageToPath()}/web/rest/"){
                ant.include(name: "**/*.*")
                ant.exclude(name: "*.*")
            }
        }
        ...
        // 替换 package
        replaceText(/^\s*package\s+${this.groupId}\.domain\.([\w\.\d]+)\s*;/, /package\s+${projectGroupId}.application.service.dto.\1;/, "gm",
        ...
        copyPomXml("$targetDir/pom.xml", "${subProjectDir}/${artifactId.dashCase()}-web/pom.xml",
            ...
            // 移除所有的plugin，保留 compiler
            removePomNode(pom.build, pom.build.defaultGoal)
            removePomNode(pom.build, pom.build.pluginManagement)

            removeNodeChild(pom.build.plugins){
...
service * with serviceImpl
    ...
    void processEachEntity(String entityName, def entityOptions, def relationNames, def reverseRelationNames, def absfiles){
        ...
        // 去掉 pg-xxx 的所有内容，避免污染代码
        replaceText(PG_PATTERN, "", "gm", domainFiles)
    ...
    void postProcess(){
        ...
        // 替换 package
        replaceText(/^\s*package\s+${this.groupId.toRegexp()}\.domain\.([\w\.\d]+)\s*;/, /package ${projectGroupId}.application.domain.\1;/, "gm",
                cloneFilesetMapArray([[
                        dir:"$tempSrcJavaDir/${groupId.packageToPath()}/domain/",
                        includes:["**/*.java"]
                ]]))
...
service * with serviceImpl
    ...
    void postProcess(){
        ...
        // 替换 package
        replaceText(/^\s*package\s+${this.groupId.toRegexp()}\.domain\.([\w\.\d]+)\s*;/, /package ${groupId}.po.\1;/, "gm",
                cloneFilesetMapArray([[
                                              dir:"$tempSrcJavaDir/${groupId.packageToPath()}/domain/",
                                              includes:["**/*.java"]
                                      ]]))
